...
def encode_proxy_mp4(
    ...
    try:
        ...
        _cleanup_partial_files(output_path, jpeg_location, context)
        raise
...
def _set_encoding_status(db, file_id: int) -> None:
    with db.get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE app.file_catalogue SET file_status = 'encoding', "
...
def _cleanup_partial_files(
    ...
    jpeg_location: str | None,
    context: OpExecutionContext,
) -> None:
    for label, path in [("proxy", output_path), ("JPEG", jpeg_location)]:
