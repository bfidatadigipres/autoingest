...
def encode_histogram(encode_data: list[dict]) -> go.Figure:
    ...
    fig.update_layout(legend=dict(orientation="h", y=1.0, yanchor="bottom"))
    return fig
...
def error_bar(error_counts: list[tuple]) -> go.Figure:
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", y=1.14, yanchor="bottom"),
    )
    return fig
...
def storage_filetype_blocks(filetype_data: list[tuple], storage: str = None) -> go.Figure:
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", y=1.14, yanchor="bottom", itemsizing="constant"),
    )
    return fig
