...
def main():
    ...
    bucket, bucket_list = bp.get_buckets(bucket_collection, False)
    ...
    if not files:
        for folder in folders:
            ...
            if job_list:
                ...
                new_fpath = pth_rename(folderpth, validate_path, job_list)
                if not new_fpath:
                    logger.warning("Renaming of folderpath to job id failed.")
                    logger.warning(
                        "Please ensure this folder %s is renamed manually to %s %s",
                        folderpth,
                        validate_path,
                        job_list,
                    )
                    ...
                    fail_list = update_job_id_postgres(new_fpath)
                    if len(fail_list) > 0:
                        for fail in fail_list:
                            logger.warning("%s - PostgreSQL row for filename was not updated with job ID: %s", fail, os.path.basename(new_fpath))
                    else:
                        logger.info("All PostgreSQL file rows in JOB %s updated", os.path.basename(new_fpath))
...
def pth_rename(folderpth: str, validate_path: str, job_list: list[str]) -> Optional[str]:
    """
...
def update_job_id_postgres(new_fpath: str) -> list[str]:
    """
    ...
    file_list = [x for x in os.listdir(new_fpath) if os.path.isfile(os.path.join(new_fpath, x))]
